...
{#if $CurrentlyPlaying && $CurrentlyPlaying.is_playing}
	{#if $ProgressData}
		<div
			...
			class="transition-colors flex flex-col sm:flex-row justify-center items-center text-white p-2 rounded-t-2xl border-2 w-fit bg-black bg-opacity-50"
			style="border-color: {$HoveredColour};"
		...
		>
			{#key $CurrentlyPlaying.item.name}
				...
				>
				<div
					class="flex flex-col text-2xl px-2 sm:px-8 justify-center text-ellipsis text-center sm:text-start items-center sm:items-start pt-2 sm:pt-0"
				>
					<a class="font-bold hover:underline w-fit" href={$CurrentlyPlaying.item.uri}
						><h2 class="transition-colors" bind:this={colorText}>
